...
function buildCategoryCarousels(all) {
  const el = document.getElementById("categoryCarousels");
  if (!el) return; // this page has no carousel container (e.g. category.html)
  ...
  // Order categories by most recently updated first; each list is already
  // sorted newest-first because articles.json is built that way.
  const categories = [...byCategory.keys()].sort(
    (a, b) => (byCategory.get(a)[0].date < byCategory.get(b)[0].date ? 1 : -1)
  );
      ...
      return `
        <section class="category-section">
          <div class="category-section-head">
            ...
            <a href="/category.html?cat=${encodeURIComponent(slug)}">View all ${escapeHtml(cat)} →</a>
          </div>
          <div class="carousel-track">${items.map(carouselCardHtml).join("")}</div>
        </section>`;
    })
    .join("");
}
...
fetch("/articles.json", { cache: "no-store" })
  ...
  .then((data) => {
    ...
    buildCategoryCarousels(state.all);
