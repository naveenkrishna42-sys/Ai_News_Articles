// TIVRA News â€” article page template.
...
/**
 * buildHeroCredit â€” format credit line for hero image.
 * Accepts either:
...
export function buildHeroCredit(heroCredit) {
  ...
  if (typeof heroCredit === "string" || !heroCredit) {
    ...
    if (credit && credit.startsWith("Wikimedia")) {
      ...
      return `Representative image Â· ${escapeHtml(credit)} (free license)`;
    } else {
...
/**
 * renderComparisonTable â€” HTML for a gadget comparison / ranking spec table.
 ...
 * text) â€” no new colours introduced.
 *
 ...
 * two-column table never overflows a narrow (e.g. 360px) viewport â€” it just
 ...
 * signature â€” callers concatenate its output into bodyHtml themselves.
 */

/**
 * specTableHasData â€” true if at least one row has a real, non-null value.
 ...
 * nulls every single row â€” the caller should check this BEFORE calling
...
// Honest replacement for an all-null spec table â€” same visual weight as the
...
// expected periscope camera"), pass them as `highlights` â€” they're rendered
...
export function buildAwaitingSpecsNotice(highlights) {
  const highlightsHtml = highlights && String(highlights).trim()
    ...
    : "";
  return `${highlightsHtml}<div style="margin:20px 0;padding:14px 18px;background:#fffbeb;border:1px solid #fde68a;border-radius:10px;font-size:.9rem;color:#92400e;">Official specifications haven't been confirmed yet â€” this article will be updated with full details once they're announced. Check the source report below for the latest.</div>`;
}
...
export function renderArticlePage({
  ...
  // NewsArticle structured data â€” what search engines and Google Discover
...
<div class="wrap">
  <div class="crumb"><a href="/">Home</a> â€º <a href="/category.html?cat=${catSlug}">${escapeHtml(category)}</a></div>
  <span class="cat-pill">${escapeHtml(category)}</span>
  <h1>${safeTitle}</h1>
  <div class="meta">
    <span>ðŸ“… ${date}</span>
    <span>ðŸ“‚ ${escapeHtml(category)}</span>
    ${sourceUrl ? `<a href="${escapeHtml(sourceUrl)}" target="_blank" rel="noopener noreferrer nofollow">Original source â†—</a>` : ""}
  </div>
...
${bodyHtml}
  ...
  <div class="srcbox">
    ...
    Reported by ${escapeHtml(sourceName || "news agencies")}. This article was written with AI assistance from publicly available reporting â€” always cross-check important details with the original coverage.
    <div class="links">
  ...
  <div class="notice">This content is AI-assisted and published for information only. TIVRA News links every story to its original source above â€” please verify dates, figures and statements there. See our <a href="/disclaimer.html" style="color:#92400e;font-weight:700;">Disclaimer</a> and <a href="/editorial-policy.html" style="color:#92400e;font-weight:700;">Editorial Policy</a>.</div>
</div>
<footer class="site">
...
<script type="text/javascript" src="https://s.skimresources.com/js/308051X1796347.skimlinks.js"></script>
</body>
</html>`;
}
